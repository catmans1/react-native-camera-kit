    ...
    for(AVMetadataObject *metadataObject in metadataObjects)
    {
        if ([metadataObject isKindOfClass:[AVMetadataMachineReadableCodeObject class]] && [self isSupportedBarCodeType:metadataObject.type]) {
            ...
            if (self.onReadCode && code.stringValue && ![code.stringValue isEqualToString:self.codeStringValue]) {
                self.onReadCode(@{@"codeStringValue": code.stringValue, @"codeType": [self convertToFormat:code.type]});
                [self stopAnimatingScanner];
            }
        }
    }
...
- (BOOL)isSupportedBarCodeType:(NSString *)currentType {
    BOOL result = NO;
    NSMutableArray *supportedBarcodeTypes = [[NSMutableArray alloc] initWithArray:@[
    ...
    if (@available(iOS 15.4, *)) {
        [supportedBarcodeTypes addObject:AVMetadataObjectTypeCodabarCode];
        [supportedBarcodeTypes addObject:AVMetadataObjectTypeGS1DataBarCode];
        [supportedBarcodeTypes addObject:AVMetadataObjectTypeGS1DataBarExpandedCode];
        [supportedBarcodeTypes addObject:AVMetadataObjectTypeGS1DataBarLimitedCode];
        [supportedBarcodeTypes addObject:AVMetadataObjectTypeMicroPDF417Code];
        [supportedBarcodeTypes addObject:AVMetadataObjectTypeMicroQRCode];
    }
...
- (NSString *)convertToFormat:(NSString *)format {
    
    if ([format isEqualToString:AVMetadataObjectTypeUPCECode]) {
        return @"FORMAT_UPC_E";
    }
    if ([format isEqualToString:AVMetadataObjectTypeCode39Code]){
        return @"FORMAT_CODE_39";
    }
    if ([format isEqualToString:AVMetadataObjectTypeCode39Mod43Code]){
        return @"FORMAT_CODE_39_MOD_43";
    }
    if ([format isEqualToString:AVMetadataObjectTypeInterleaved2of5Code]){
        return @"FORMAT_ITF";
    }
    if ([format isEqualToString:AVMetadataObjectTypeITF14Code]){
        return @"FORMAT_ITF_14";
    }
    if ([format isEqualToString:AVMetadataObjectTypeEAN13Code]){
        return @"FORMAT_EAN_13"; // include FORMAT_UPC_A
    }
    if ([format isEqualToString:AVMetadataObjectTypeEAN8Code]){
        return @"FORMAT_EAN_8";
    }
    if ([format isEqualToString:AVMetadataObjectTypeCode93Code]){
        return @"FORMAT_CODE_93";
    }
    if ([format isEqualToString:AVMetadataObjectTypeCode128Code]){
        return @"FORMAT_CODE_128";
    }
    if ([format isEqualToString:AVMetadataObjectTypePDF417Code]){
        return @"FORMAT_PDF417";
    }
    if ([format isEqualToString:AVMetadataObjectTypeQRCode]){
        return @"FORMAT_QR_CODE";
    }
    if ([format isEqualToString:AVMetadataObjectTypeAztecCode]){
        return @"FORMAT_AZTEC";
    }
    if ([format isEqualToString:AVMetadataObjectTypeDataMatrixCode]){
        return @"FORMAT_DATA_MATRIX";
    }
    if (@available(iOS 15.4, *)) {
        if ([format isEqualToString:AVMetadataObjectTypeCodabarCode]) {
            return @"FORMAT_CODABAR";
        }
        if ([format isEqualToString:AVMetadataObjectTypeGS1DataBarCode]) {
            return @"FORMAT_GS1";
        }
        if ([format isEqualToString:AVMetadataObjectTypeGS1DataBarExpandedCode]) {
            return @"FORMAT_GS1_EXPANDED";
        }
        if ([format isEqualToString:AVMetadataObjectTypeGS1DataBarLimitedCode]) {
            return @"FORMAT_GS1_LIMITED";
        }
        if ([format isEqualToString:AVMetadataObjectTypeMicroPDF417Code]) {
            return @"FORMAT_MICRO_PDF417";
        }
        if ([format isEqualToString:AVMetadataObjectTypeMicroQRCode]) {
            return @"FORMAT_MICRO_QR_CODE";
        }
    }
